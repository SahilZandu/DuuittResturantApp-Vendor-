  ...
  const CheckoutButton = () => {
    return (
      <View
        ...
        <SvgXml xml={appImages.rightIcon} fill="#FFF" />
      </View>
    );
  };
        ...
        <View style={{justifyContent: 'center', alignItems: 'center'}}>
          ...
          {timerCount > 0 ? (
            ...
            <>
              <Text
                style={
                  styles.orderAcceptText
                }>{`Your order cooking time is over by ${Number(
                item?.cooking_time,
              )} minutes, resulting in a delay for this order.`}</Text>
            </>
          )}
        </View>
      )}
